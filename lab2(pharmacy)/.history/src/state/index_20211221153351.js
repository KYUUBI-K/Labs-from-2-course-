...
const store = new Vuex.Store({
  ...
  mutations:{
    ...
    deleteTask(state,dataobj){
        state.taskList = state.taskList.filter(item=>(item.id != dataobj.id))
        state.taskList = state.taskList.filter(item=>(item.title != dataobj.title))
        state.taskList = state.taskList.filter(item=>(item.price != dataobj.price))
        state.taskList = state.taskList.filter(item=>(item.active_substance != dataobj.active_substance))
    }
  },
  actions:{
    addTask({ commit }, data) {
